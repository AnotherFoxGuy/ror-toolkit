...
class MainFrame(wx.Frame):
    ...
    def __init__(self, parent, id=-1, title="", pos=wx.DefaultPosition,
        ...
        if rorSettings().has_section("MainFrame"):
            self.SetSize(  # int x, int y, int width, int height, int sizeFlags = wxSIZE_AUTO)
        ...
        if rorSettings().rorDebug: options_menu.AppendItem(
            FM.FlatMenuItem(options_menu, ID_ModTool, "mod Tool Dependences", wx.ITEM_NORMAL))
        if rorSettings().rorDebug: options_menu.AppendItem(
            FM.FlatMenuItem(options_menu, ID_wxInspector, "wx Widget Inspector", "", wx.ITEM_NORMAL))
        if rorSettings().rorDebug: options_menu.AppendItem(
            FM.FlatMenuItem(options_menu, ID_hideCaptions, "hide toolbar captions", "", wx.ITEM_NORMAL))
        ...
        self._perspectives_menu.AppendItem(
            FM.FlatMenuItem(self._perspectives_menu, ID_CreatePerspective, "Create Perspective", "", wx.ITEM_NORMAL))
        # self._perspectives_menu.Append(ID_CopyPerspective, "Copy Perspective Data To Clipboard")
        self._perspectives_menu.AppendSeparator()
        self._perspectives_menu.AppendItem(
            FM.FlatMenuItem(self._perspectives_menu, ID_FirstPerspective + 0, "Default Startup", "", wx.ITEM_RADIO))
        self._perspectives_menu.AppendItem(
            FM.FlatMenuItem(self._perspectives_menu, ID_FirstPerspective + 1, "Terrain", "", wx.ITEM_RADIO))
        self._perspectives_menu.AppendItem(
            FM.FlatMenuItem(self._perspectives_menu, ID_FirstPerspective + 2, "ODef Editor", "", wx.ITEM_RADIO))
        self._perspectives_menu.AppendItem(
            FM.FlatMenuItem(self._perspectives_menu, ID_FirstPerspective + 3, "Truck Editor", "", wx.ITEM_RADIO))
        ...
        #				self.terraintoolbar.AddLabelTool(ID_OpenTerrain, "Open Terrain", wx.ArtProvider.GetBitmap(wx.ART_FILE_OPEN))
        self.terraintoolbar.AddTool(ID_SaveTerrain, "Save Terrain", wx.ArtProvider.GetBitmap(wx.ART_FILE_SAVE))
        self.terraintoolbar.AddTool(ID_SaveTerrainAs, "Save Terrain As", wx.ArtProvider.GetBitmap(wx.ART_FILE_SAVE_AS))
        self.terraintoolbar.AddSeparator()
        #				self.terraintoolbar.AddLabelTool(ID_StickTo02, "Stick to 0.2 m",wx.ArtProvider.GetBitmap(wx.ART_GO_DOWN),
        #									shortHelp="Stick New Objects to 0.2 from ground")
        #				self.terraintoolbar.AddLabelTool(ID_Inspector, "Object Inspector",wx.ArtProvider.GetBitmap(wx.ART_GO_DOWN),
        #									shortHelp="Stick New Objects to 0.2 from ground")
        ...
        # self.terraintoolbar.AddTool(ID_AddObject, "Add Something", wx.ArtProvider.GetBitmap(wx.ART_NEW))
        # self.terraintoolbar.EnableTool(ID_AddObject, False)
        # self.terraintoolbar.AddTool(ID_DeleteSelection, "Delete Selection", wx.ArtProvider.GetBitmap(wx.ART_DELETE))
        # self.terraintoolbar.EnableTool(ID_DeleteSelection, False)
        # self.terraintoolbar.AddSeparator()
        self.terraintoolbar.AddTool(ID_CopySelection, "Copy Selection", wx.ArtProvider.GetBitmap(wx.ART_COPY))
        #				self.terraintoolbar.EnableTool(ID_CopySelection, False)
        self.terraintoolbar.AddTool(ID_PasteSelection, "Paste Selection", wx.ArtProvider.GetBitmap(wx.ART_PASTE))
        #				self.terraintoolbar.EnableTool(ID_PasteSelection, False)
        # self.terraintoolbar.AddSeparator()
        # self.terraintoolbar.AddTool(ID_UndoAction, "Undo last Action", wx.ArtProvider.GetBitmap(wx.ART_UNDO))
        # self.terraintoolbar.EnableTool(ID_UndoAction, False)
        # self.terraintoolbar.AddTool(ID_RedoAction, "Redo last Undo", wx.ArtProvider.GetBitmap(wx.ART_REDO))
        # self.terraintoolbar.EnableTool(ID_RedoAction, False)
        # self.terraintoolbar.AddSeparator()
        self.terraintoolbar.AddTool(ID_FindObject, "go to...", wx.ArtProvider.GetBitmap(wx.ART_FIND),
                                    shortHelp="go to to some coordenates")
        #				self.terraintoolbar.EnableTool(ID_FindObject, True)
        # self.terraintoolbar.AddSeparator()
        #				self.terraintoolbar.AddTool(ID_Quit, "Quit", wx.ArtProvider.GetBitmap(wx.ART_QUIT))
        ...
        for w in wx.GetTopLevelWindows():
            if isinstance(w, ShapedWindow):
                ...
                buttonidx = self.terraintoolbar.AddTool(id, label, bitmap1=getBitmap(label), bitmap2=getBitmap(label),
                                                        kind=wx.ITEM_CHECK)
                self.Bind(wx.EVT_MENU, self.OnToolbarButtonClick, id=id)
                w.Hide()
                self.tbbuttons.append(
                    {'id': id, 'label': label, 'window': w, 'perspective': w.perspective, 'buttonidx': buttonidx})
        ...
        self._mgr.AddPane(self.odefEditorOgreWin,
                          wx.aui.AuiPaneInfo().Name("ogre_odef_editor_content").CenterPane().Hide())

        # the truck editor window
        self.truckEditorOgreWin = RoRTruckOgreWindow(self, wx.ID_ANY)
        self._mgr.AddPane(self.truckEditorOgreWin,
                          wx.aui.AuiPaneInfo().Name("ogre_truck_editor_content").CenterPane().Hide())
        ...
        self._mgr.AddPane(self.truckEditorUVOgreWin,
                          wx.aui.AuiPaneInfo().Name("ogre_truck_editor_uv_content").Float().Hide())
    ...
    def OnToolbarButtonClick(self, evt):
        ...
        if idx > -1:
            if self.tbbuttons[idx]['window'].IsShown():
                ...
                self.tbbuttons[idx]['window'].Show(True)
    ...
    def OnStartRoRClick(self,
        ...
        if self.terrainOgreWin:
            if self.terrainOgreWin.terrain:
                ...
                log().info(
                    "command line: " + rorSettings().rorFile + " -map " + self.terrainOgreWin.terrain.name + truck)
                #					Popen(rorSettings().rorFile + " -map " +  self.terrainOgreWin.terrain.name, 1024, cwd = rorSettings().rorFolder )
                call(rorSettings().rorFile + " -map " + self.terrainOgreWin.terrain.name, 1024,
                     cwd=rorSettings().rorFolder)
    ...
    def OnSaveTerrain(self, event=None):
        if self.lastFilenameUsed == "":
            ...
            dialog = wx.FileDialog(self, "Save Terrain as", default, "", "Terrain Files (*.terrn)|*.terrn",
